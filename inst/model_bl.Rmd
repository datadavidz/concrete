---
title: "Create the XGBoost Model for Concrete dataset"
author: "datadavidz"
date: "4/9/2021"
...
Load the original dataset and reformat the column names
```{r, include=FALSE}
numberCols <- 9 #total number of columns in spreadsheet

colTypes <- rep("numeric", numberCols)

concrete_tbl <- read_excel(path = here::here("data-raw/Concrete_Data.xls"), col_types = colTypes)
...
```{r}
final_res <- last_fit(final_xgboost, split = concrete_split, metrics = metric_set(rmse, rsq, mae))
```
...
Create final model for future predictions
```{r}
model <- fit(final_xgboost, concrete_tbl)
```


Save model to package data directory
```{r}
